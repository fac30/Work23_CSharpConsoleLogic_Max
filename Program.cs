        ...
        case "3":
            ...
            for (int i = 0; i < maxPets; i++)
            {
                if (ourAnimals[i, 0] != "ID #: ")
                {
                    ...
                    animalAge = ourAnimals[i, 2].Substring(ourAnimals[i, 2].IndexOf(": ") + 1);
                    animalPhysicalDescription = ourAnimals[i, 4].Substring(ourAnimals[i, 4].IndexOf(": ") + 1);
                    ...
                    while (!validEntry)
                    {
                        Console.WriteLine($"Enter an physical description for ID #: {animalID} (size, color, gender, weight, housebroken)");
                        readResult = Console.ReadLine();
